...
//////////////////////////////////////////////////////////////
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (action) {
                ...
                BluetoothLeService.ACTION_DATA_AVAILABLE -> {
                    //println(intent.getStringExtra(BluetoothLeService.EXTRA_DATA))
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tracker = LocationTracker(this@DeviceControlActivity)
        ...
        val int_time: Button = findViewById(R.id.IntTime)

        int_time.setOnClickListener({
            IntRead()
        })


        val intnum: EditText = findViewById(R.id.intNum)
        intnum.filters = arrayOf<InputFilter>(InputFilterMinMax("1", "255"))
    }
    ...
    fun IntRead() {
        ...
        Thread.sleep(1000)
        val intnum: EditText = findViewById(R.id.intNum)
        println(intnum.text.toString())
        char2?.setValue(intnum.text.toString())
        mBluetoothLeService?.writeCharacteristic(char2!!)
        Thread.sleep(500)
        mBluetoothLeService?.readCharacteristic(char2!!)
        Thread.sleep(500)
        Toast.makeText(this, BluetoothLeService.integrationTime, Toast.LENGTH_LONG).show()
        scanning = true
    ...
    fun SendClick() {
        ...
        val urlString = "https://api.objectsyndicate.com/api/v1/luci/post/"
        ...
        val send = SendTask().execute(sendURL).get()
        Toast.makeText(this, send.toString(), Toast.LENGTH_LONG).show()

    }
////
    private inner class SendTask : AsyncTask<URL, Int, Int>() {
        ...
        override fun doInBackground(vararg urls: URL): Int? {
            ...
            try {
                ...
                val LuciToken = userTokenPref?.getString("LuciToken", "")


                conn.doOutput = true
                conn.doInput = true
                conn.setRequestProperty("Content-Type", "application/json")
                conn.setRequestProperty("Authorization", "Token " + LuciToken)
                conn.requestMethod = "POST"
                conn.connectTimeout = 1 * 60 * 1000


                val s450 = "%.2f".format(v450)
                val s500 = "%.2f".format(v500)
                val s550 = "%.2f".format(v550)
                val s570 = "%.2f".format(v570)
                val s600 = "%.2f".format(v600)
                val s650 = "%.2f".format(v650)

                /*
                val s650 = "%.2f".format(v650)
                val s680 = "%.2f".format(v680)
                val s730 = "%.2f".format(v730)
                val s760 = "%.2f".format(v760)
                val s810 = "%.2f".format(v810)
                val s860 = "%.2f".format(v860)
*/

                val jsonRequest = "{\"location\":\"${latitude}_${longitude}\", \"wm450\":$s450, \"wm500\":$s500, \"wm550\":$s550, \"wm570\":$s570, \"wm600\":$s600, \"wm650\":$s650}"
                println(jsonRequest)
        ...
        if (data != null) {
            ...
            try {
                ...
                ppfdv.text = String.format("%,.2f",(VioletMol+BlueMol+GreenMol+OrangeMol+RedMol)  ) + " μMole/m²"
                    ...
                    val rainArray: IntArray = intArrayOf(Color.MAGENTA, Color.argb(255,127,0,255), Color.BLUE, Color.GREEN, Color.YELLOW, Color.argb(255,255,170,0), Color.RED, Color.argb(255,170,0,0), Color.BLACK, Color.BLACK, Color.BLACK, Color.BLACK)
...
*/
            ...
            for (gattCharacteristic in gattCharacteristics) {
                ...
                when (uuid) {
                    ...
                    SampleGattAttributes.ARDUINO_INFO -> {
                        // Arduino
                    }
                    SampleGattAttributes.ARDUINO -> {
                        // Arduino
                    }
                    SampleGattAttributes.ARDUINO2 -> {
                        // Arduino
                    }
                    else -> {
                        //println(uuid)
            ...
            fixedRateTimer = fixedRateTimer(name = "high-timer",
                ...
                char2 = mGattCharacteristics!![0][1]
            ...
            //println(gattCharacteristicGroupData)
